...
void launchArgmax(std::byte *max_idx, std::byte *max_val, const std::byte *vals, size_t vals_size) {
    ...
    if (vals_size == 0) {
        ...
        checkCudaError(musaGetLastError(), "setEmptyArgmaxKernel");
        return;
    }
    ...
    if (block_count == 1) {
        ...
        checkCudaError(musaGetLastError(), "argmaxReduceKernel");
        return;
    }
    ...
    checkCudaError(musaMalloc(&current_idxs, block_count * sizeof(int64_t)), "musaMalloc");
    checkCudaError(musaMalloc(&current_vals, block_count * sizeof(T)), "musaMalloc");
    ...
    while (current_count > 1) {
        ...
        if (next_count == 1) {
            argmaxReduceKernel<<<1, block_size, shared_size>>>(out_idx, out_val, current_idxs, current_vals,
                                                               current_count);
            checkCudaError(musaGetLastError(), "argmaxReduceKernel");
            checkCudaError(musaFree(current_idxs), "musaFree");
            checkCudaError(musaFree(current_vals), "musaFree");
            return;
        }

        int64_t *next_idxs = nullptr;
        T *next_vals = nullptr;
        checkCudaError(musaMalloc(&next_idxs, next_count * sizeof(int64_t)), "musaMalloc");
        checkCudaError(musaMalloc(&next_vals, next_count * sizeof(T)), "musaMalloc");
